...
export default function CrmPipeline() {
  ...
  const handleMoveOpportunity = async (id: string, newStage: CrmOpportunityStage) => {
    ...
    try {
      ...
      toast.error(e instanceof Error ? e.message : "Error al mover la oportunidad");
      mutate();
    }
  };

  const handleOpenOpportunityDetail = (opp: CrmOpportunity) => {
    setDetailOpportunityId(opp.id);
    setDetailOpen(true);
  };

  const handleDetailOpenChange = (open: boolean) => {
    setDetailOpen(open);
    if (!open) setDetailOpportunityId(null);
  };
            ...
            <>
              {view === "kanban" ? (
                <CrmKanbanBoard
                  ...
                  onOpenOpportunityDetail={handleOpenOpportunityDetail}
                />
              ) : (
                <CrmPipelineTable
                  opportunities={opportunities}
