...
import { CP_MODULE, examSalesLineUpdateSchema } from "@/lib/coordinacion-proyectos/schemas";
...
export const PATCH = withAuth(async (req, { supabase, user, member }, ctx: RouteCtx) => {
    ...
    const parsed = examSalesLineUpdateSchema.safeParse(body);
    if (!parsed.success) {
...
export const DELETE = withAuth(async (_req, { supabase, user, member }, ctx: RouteCtx) => {
    ...
    const { error } = await supabase.from("lec_exam_sales_lines").delete().eq("org_id", member.org_id).eq("id", id);
    if (error) return NextResponse.json({ error: error.message }, { status: 500 });
