...
import { ArrowLeft, FileText, Clock, Users, MapPin, Calculator, Mail, Download, Pencil, ChevronDown, ChevronUp, Info } from "lucide-react";
                                        ...
                                        <div className="grid grid-cols-1 md:grid-cols-2 divide-y md:divide-y-0 md:divide-x">
                                            ...
                                            <div className="p-6 space-y-4 bg-muted/10">
                                                ...
                                                <div className="pt-2 border-t border-dashed">
                                                    <p className="text-xs font-medium text-foreground mb-2 uppercase tracking-wide">Staff (Invigilators):</p>
                                                    {invigilators.length > 0 ? (
                                                        <div className="flex flex-wrap gap-2">
                                                            {invigilators.map((s: any) => (
                                                                <Badge key={s.id} variant="outline" className="bg-background">{s.applicator?.name}</Badge>
                                                            ))}
                                                        </div>
                                                    ) : (
                                                        <span className="text-xs text-destructive flex items-center gap-1"><span className="h-2 w-2 rounded-full bg-destructive animate-pulse" /> Sin asignar</span>
                                                    )}
                                                </div>
                                            </div>
                                            ...
                                            <div className="p-6 space-y-4 bg-muted/10">
                                                ...
                                                <div className="pt-2 border-t border-dashed">
                                                    <p className="text-xs font-medium text-foreground mb-2 uppercase tracking-wide">Staff (Speaking Evaluators):</p>
                                                    {evaluators.length > 0 ? (
                                                        <div className="flex flex-wrap gap-2">
                                                            {evaluators.map((s: any) => (
                                                                <Badge key={s.id} variant="outline" className="bg-background border-emerald-500/30">{s.applicator?.name}</Badge>
                                                            ))}
                                                        </div>
                                                    ) : (
                                                        <span className="text-xs text-destructive flex items-center gap-1"><span className="h-2 w-2 rounded-full bg-destructive animate-pulse" /> Sin asignar</span>
