-- Migration: 20260512_invitation_seed_module_access.sql
-- Problem: fn_accept_invitation creates org_members but NOT member_module_access rows.
--          New members get 0 permission rows → sidebar shows no modules.
-- Fix:     After creating the member, seed member_module_access from module_registry
--          using role-based defaults.
...
BEGIN
    ...
    -- ──────────────────────────────────────────────────────────────────────
    -- NEW: Seed member_module_access from module_registry using role defaults
    -- ──────────────────────────────────────────────────────────────────────
    -- Role-based defaults:
    --   admin       → can_view=T, can_edit=T, can_delete=T  (all modules)
    --   supervisor  → can_view=T, can_edit=T, can_delete=F
    --   operador    → can_view=T, can_edit=F, can_delete=F
    --   applicator  → no rows (uses portal, not dashboard modules)
    --
    -- Modules come from module_registry where:
    --   is_native=true AND (org_id IS NULL OR org_id = invitation.org_id)
    -- ──────────────────────────────────────────────────────────────────────
    IF v_invitation.role != 'applicator' THEN
        ...
        SELECT
            ...
            v_member_id,
            mr.slug,
            true,  -- can_view: all non-applicator roles can view
        ...
        WHERE mr.is_native = true
          ...
          -- Exclude meta-modules that don't need access rows
