-- Migration: 20260501_sgc_audit_checks.sql
-- Description: Adds persistent SGC audit checklist table with tenant RLS and starter data.
...
CREATE INDEX IF NOT EXISTS idx_sgc_audit_checks_org_sort
    ON public.sgc_audit_checks(org_id, sort_order, clause_id);

ALTER TABLE public.sgc_audit_checks ENABLE ROW LEVEL SECURITY;

CREATE POLICY "sgc_audit_checks_select"
ON public.sgc_audit_checks FOR SELECT TO authenticated
USING (
    org_id IN (
...
CREATE POLICY "sgc_audit_checks_insert"
ON public.sgc_audit_checks FOR INSERT TO authenticated
WITH CHECK (
    org_id IN (
...
CREATE POLICY "sgc_audit_checks_update"
ON public.sgc_audit_checks FOR UPDATE TO authenticated
USING (
    org_id IN (
        SELECT org_id FROM public.org_members
        WHERE user_id = auth.uid() AND role IN ('admin', 'supervisor')
...
WITH base(clause_id, title, question, tags, sort_order) AS (
    ...
    ('7.2', 'Competencia', 'Se asegura que el personal es competente basado en educacion y experiencia?', ARRAY['ISO 9001', 'ISO 21001']::TEXT[], 9),
    ...
    ('9.1', 'Satisfaccion del cliente y estudiante', 'Se realiza seguimiento a las percepciones de los estudiantes sobre el cumplimiento de sus necesidades?', ARRAY['ISO 9001', 'ISO 21001']::TEXT[], 12),
...
INSERT INTO public.sgc_audit_checks (
    org_id,
    clause_id,
    title,
    question,
